...
if ($arch eq "x86_64") {
	...
	# 130:	00 00 84 01 	taddpcil	$t4,0x0
	#			130: R_GPTX_T_PAGE_HIGH	_mcount
	# 134:	00 40 20 01 	taddti	$t0,$t4,0x0
	#			134: R_GPTX_T_PAGE_LOW	_mcount
	# 138:	00 00 47 02 	call	$t0
    
    $alignment = 3;
#   $mcount_adjust = 8;
    $mcount_regex = "^\\s*([0-9a-fA-F]+): R_GPTX_T_PAGE_HIGH\\s+_mcount\$";    
    $type = ".dword";    
} else {
