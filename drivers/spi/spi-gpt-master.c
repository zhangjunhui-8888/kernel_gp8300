...
#define GPT_SPI_NAME "gpt-spi-master"
...
static void gspi_init_hwinit(struct gpt_spi_master *gspi)
{
	...
	gspi_write32(GPT_SPI_MODE_MASTER | GPT_SPI_MSB | GPT_SPI_SSN_SEL | GPT_SPI_SLV_XFER_SAME_EDGE,regs_base + GPT_SPI_CFG);
	gspi_write32(200,regs_base + GPT_SPI_DIV_CNT);
...
{
	struct gpt_spi_master *gspi = dev_id;
	u32 value = 0;
...
{
	struct gpt_spi_master *gs = spi_master_get_devdata(master);
	u32 value  = 0;
	unsigned int cfg = 0;
	...
	{
		gs->len = tfr->len;	
		gs->tx_buf8 = tfr->tx_buf;
		gs->rx_buf8 = tfr->rx_buf;

	}else if(gs->cur_bpw == 16){
		gs->len = tfr->len / 2;
		gs->tx_buf16 = tfr->tx_buf;
		gs->rx_buf16 = tfr->rx_buf;

	}else if(gs->cur_bpw == 32){
		gs->len = tfr->len / 4;
		gs->tx_buf32 = tfr->tx_buf;
		gs->rx_buf32 = tfr->rx_buf;
	}
	...
	if(0 != count)
	{
		...
		while(count--)
		{
			...
			while(inter_cnt--)
			{
				if(gs->cur_bpw == 8)
				{
					*gs->rx_buf8 = gspi_read32(gs->regs + GPT_SPI_RX_DATA);
					gs->rx_buf8++;

				}else if(gs->cur_bpw == 16){
					*gs->rx_buf16 = gspi_read32(gs->regs + GPT_SPI_RX_DATA);
					gs->rx_buf16++;

				}else if(gs->cur_bpw == 32){
					*gs->rx_buf32 = gspi_read32(gs->regs + GPT_SPI_RX_DATA);
					gs->rx_buf32++;
				}
			}
...
static int gpt_spi_config(struct gpt_spi_master *gs, struct spi_device *spi)
{
	...
	value = (gs->apiu_clk / 2 / gs->cur_speed) - 1;	
	...
	spi->max_speed_hz = gs->apiu_clk / 2 / (value + 1);
...
{
	struct gpt_spi_master *gs =  spi_master_get_devdata(sm);
...
{
	struct gpt_spi_master *gs = spi_master_get_devdata(spi->master);
...
static int gpt_spi_probe(struct platform_device *pdev)
{
	...
	master = spi_alloc_master(&pdev->dev, sizeof(struct gpt_spi_master));
	if (!master)
	{
		return -ENODEV;
	}
	...
	gspi->clk = devm_clk_get(&pdev->dev, NULL);
	if (IS_ERR(gspi->clk))
		return PTR_ERR(gspi->clk);
	
	ret = clk_prepare_enable(gspi->clk);
	if (ret)
		return ret;

	gspi->apiu_clk = clk_get_rate(gspi->clk);
...
static int gpt_spi_remove(struct platform_device *pdev)
{
	...
	struct gpt_spi_master *gspi = spi_master_get_devdata(master);

	iounmap((void __iomem*)gspi->regs);
	clk_disable_unprepare(gspi->clk);
...
MODULE_DESCRIPTION("Gpt Spi Master driver");
MODULE_LICENSE("GPL");
