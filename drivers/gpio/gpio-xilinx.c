...
/**
 * xgpio_irq_clean - Clean irq for gpio
 * @mm_gc: Pointer to memory mapped GPIO chip structure
 */
static void xgpio_irq_clean(struct of_mm_gpio_chip *mm_gc)
{
	/* Disable global interrupt register */
	xgpio_writereg(mm_gc->regs + XGPIO_GIER_OFFSET, 0);
	/* Disable channel irq */
	xgpio_writereg(mm_gc->regs + XGPIO_IPIER_OFFSET, 0);
	/* Clean status register */
	xgpio_writereg(mm_gc->regs + XGPIO_IPISR_OFFSET, xgpio_readreg(mm_gc->regs + XGPIO_IPISR_OFFSET));
}
...
static void xgpio_save_regs(struct of_mm_gpio_chip *mm_gc)
{
	struct xgpio_instance *chip =
	    container_of(mm_gc, struct xgpio_instance, mmchip);
	/* Clean interrupt registers, it maybe caused a interrupt on first startup */
	xgpio_irq_clean(mm_gc);
		       ...
		       const struct of_phandle_args *gpiospec, u32 *flags)
{
	...
	/* We can support 3 gpio cells: bank, pin, flags */
	u32 bank = gpiospec->args[0];
	if ((bank * XGPIO_CHANNEL_OFFSET) != chip->offset)
		return -EINVAL;
...
#endif
{
	...
	/* We Can support rising edge and falling edge. */
	irqmask = (val ^ chip->gpio_state) & chip->irq_enable;
	chip->gpio_state = val;	/* save new val */
	if (irqmask) {
		for_each_set_bit(offset, &irqmask, chip->mmchip.gc.ngpio) {
			generic_handle_irq(chip->irq_base + offset);
		}
	}
	else {
		/* FIXME: if not add this, the irq has be set ON by HW, it will always ON and cannot be clear. */
		udelay(10);
	}
...
static int xgpio_of_probe(struct platform_device *pdev)
{
	...
	chip->mmchip.gc.of_gpio_n_cells = XGPIO_N_CELLS;
