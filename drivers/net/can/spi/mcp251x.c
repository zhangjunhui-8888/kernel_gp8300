...
static int mcp251x_hw_reset(struct spi_device *spi)
{
	...
	{
		dev_err(&spi->dev,"MCP2515 don't ernter the configuration mode\n");
		return -ENODEV;
	}
	return 0;
}
...
static int mcp251x_can_probe(struct spi_device *spi)
{
	...
	if (!mcp251x_is_2510(spi)) {
		gpio = of_get_named_gpio(np, "standby", 0);
		if (gpio_is_valid(gpio)) {
			ret = devm_gpio_request_one(&spi->dev, gpio, GPIOF_OUT_INIT_LOW, "standby");
			if (ret) {
				dev_err(&spi->dev, "cannot request gpio for mcp2515 standby\n");
				goto out_clk;
			}
		}
	}
