...
static struct adv7511_link_config link_config;
...
static int adv7511_s_power(struct v4l2_subdev *sd, int on)
{
	...
	adv7511_wr_and_or(sd, 0x9a, 0xfe, 0xe0);
...
static void adv7511_setup(struct v4l2_subdev *sd)
{
	...
	v4l2_dbg(1, debug, sd, "%s\n", __func__);
#if 0
	volatile void __iomem * gpt_fb_prt = ioremap(0xf0105000, 0x40);
	if(readl(gpt_fb_prt) & 0x20){// is 16 bit.
		/* input format: rgb 4:4:4 */
		adv7511_wr_and_or(sd, 0x15, 0xf0, 0x1);
		/* Output format: RGB 4:4:4 */
		adv7511_wr_and_or(sd, 0x16, 0x00, 0xbd);
	}else{ //8 or 10 bit
		/* Input format: RGB 4:4:4 */
		adv7511_wr_and_or(sd, 0x15, 0xf0, 0x3);
		/* Output format: RGB 4:4:4 */
		adv7511_wr_and_or(sd, 0x16, 0x00, 0xb9);
	}
	iounmap(gpt_fb_prt);

#ifdef CONFIG_PLAT_GPT_CHIP2_POLARIS_DVB
		/* left align */
		adv7511_wr_and_or(sd, 0x48, 0xe7, 0x10);
#else
		/* right align */
		adv7511_wr_and_or(sd, 0x48, 0xe7, 0x00);
#endif
#else
	/* input format: rgb 4:4:4 */
	adv7511_wr_and_or(sd, 0x15, 0xf0, 0x1);
	...
	adv7511_wr_and_or(sd, 0x17, 0xf9, 0x02);
	 ...
	 * underscanned */
	adv7511_wr_and_or(sd, 0x55, 0x9f, 0x20);
	/* */
	adv7511_wr_and_or(sd, 0x40, 0xe7, 0x80);
	/* AVI Info frame packet enable, Audio Info frame disable */
	adv7511_wr_and_or(sd, 0x44, 0xe7, 0x10);
	/* Colorimetry, Active format aspect ratio: same as picure. */
	adv7511_wr(sd, 0x56, 0x08);
	/* No encryption */
	adv7511_wr_and_or(sd, 0xaf, 0xed, 0x6);
...
static int adv7511_prase_dt(struct device_node *np,
                struct adv7511_link_config *config)
{
    ...
    of_property_read_u32(np, "input-depth",&config->input_color_depth);
    if(config->input_color_depth != 8 && config->input_color_depth != 10 &&
        config->input_color_depth != 12){
        return -EINVAL;
    }
    
    ret = of_property_read_string(np, "input-colorspace", &str);
    if(ret < 0){
        return ret;
    }
    if(!__strcmp(str, "rgb")){
        config->input_colorspace = HDMI_COLORSPACE_RGB;
    } else if(!__strcmp(str, "yuv422")){
        config->input_colorspace = HDMI_COLORSPACE_YUV422;
    } else if(!__strcmp(str, "yuv444")){
        config->input_colorspace = HDMI_COLORSPACE_YUV444;
    } else {
        return -EINVAL;
    }

    ret = of_property_read_string(np, "input-clock", &str);
    if(ret < 0){
        return ret;
    }
    if(!__strcmp(str, "1x")){
        config->input_clock = ADV7511_INPUT_CLOCK_1X;
    } else if(!__strcmp(str, "2x")){
        config->input_clock = ADV7511_INPUT_CLOCK_2X;
    } else if(!__strcmp(str, "ddr")){
    ...
    if(config->input_colorspace == HDMI_COLORSPACE_YUV422 /*||
        config->input_clock != ADV7511_INPUT_CLOCK_1X*/){
        ret = of_property_read_u32(np, "input-style", &config->input_style);
        if(ret){
            return ret;
        }
        ...
        ret = of_property_read_string(np, "input-justification", &str);
        if(ret < 0){
            return ret;
        }
        if(!__strcmp(str, "left")){
            config->input_justification = ADV7511_INPUT_JUSTIFICATION_LEFT;
        } else if(!__strcmp(str, "evenly")){
            config->input_justification = ADV7511_INPUT_JUSTIFICATION_EVENLY;
        } else if(!__strcmp(str, "right")){
            config->input_justification = ADV7511_INPUT_JUSTIFICATION_RIGHT;
        } else {
            return -EINVAL;
        }
    }/* else {
    ...
    of_property_read_u32(np, "clock-delay", &config->clock_delay);
    if(config->clock_delay < -1200 || config->clock_delay > 1600){
        return -EINVAL;
    }

    config->embedded_sync = of_property_read_bool(np, "embedded-sync");
...
static void adv7511_set_link_config(struct v4l2_subdev *sd,
    ...
    color_depth = config->input_color_depth == 8 ? 3 : (config->input_color_depth == 10 ? 1 : 2);
    ...
    adv7511_wr_and_or(sd, ADV7511_REG_I2C_FREQ_ID_CFG, 0xf0, input_id); // 0x15

	/* dvb 148.5MHz:
	 * 16bit: pin 20-35; left; style1=2,style3 invert ,cby cry; inputid 1(sp) or 2(em);
	 * 8bit:  pin 20-35; left; style1 cbycry; inputid 3,4,7,8
	 * evk 74.25MHz:
	 * 16bit: pin (4-11) 16-23 28-35; evently; style1=2,style3 invert,cby cry; inputid 1 or 2
	 * 8bit:  pin 16-23 28-35; evently; evently; style1 pin16-23,style2 pin4-11; inputid 3(2xsp),4(2xem),7,8*/
    adv7511_wr_and_or(sd, ADV7511_REG_VIDEO_INPUT_CFG1, 0x00,
                (color_depth << 4) | (input_styles[config->input_style] << 2) |
				(config->input_colorspace == HDMI_COLORSPACE_RGB ? 0 : (config->input_colorspace == HDMI_COLORSPACE_YUV422 ? 1 << 7 | 1 << 0 : 1 << 0))); // 0x16
    adv7511_wr_and_or(sd, ADV7511_REG_VIDEO_INPUT_CFG2, 0xe7, config->input_justification << 3); //0x48
    adv7511_wr_and_or(sd, ADV7511_REG_TIMING_GEN_SEQ, 0xf1, config->sync_pulse << 2); // 0xd0
    adv7511_wr_and_or(sd, 0xba, 0x8f, clock_delay << 5);
    // TODO vsync hsync colorspace
	// 0x17 bit0 disable DE,
    //if(config->hsync_polarity = config->
    //adv7511_wr_and_or(sd, 0x17, 
}
static int adv7511_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ret = adv7511_prase_dt(devnod, &link_config);
	if(ret){
		return ret;
	}
...
#if 1
	if (of_property_read_u32(devnod, "edid",
                        (void *)&state->i2c_edid_addr)) {
               printk("Can't read edid value\n");
        }

	if (of_property_read_u32(devnod, "cec",
                        (void *)&state->i2c_cec_addr)) {
               printk("Can't read cec addr value\n");
        }
#else
        state->i2c_edid_addr = 0x7e;
	state->i2c_cec_addr = 0x78;
#endif
	printk("adv7511 edid(0x%x), cec(0x%x)\n", state->i2c_edid_addr, state->i2c_cec_addr);
	...
	adv7511_wr(sd, 0xe2, 0x00); /* power down cec section */
	...
	adv7511_set_link_config(sd, &link_config);
	return 0;
