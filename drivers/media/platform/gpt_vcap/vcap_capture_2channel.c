...
void init_vcap(int dev_id,int index);
...
static int vcap_start_streaming(struct vb2_queue *vq, unsigned int count)
{
		...
		int  err;
#ifdef VIDEO_DEBUG
	printk("entry %s , %d ch_id is %d.............................\n",__FUNCTION__,__LINE__,ch->channel_id);
#endif
	...
	common->set_addr(addr+ 1920*2,0x230000000,ch->channel_id+1);

	channel_first_int[VCAP_VIDEO_INDEX][ch->channel_id] = 1;

#ifndef CONFIG_SH_BINOCULAR_CAMERA
    ...
    start_capture(ch->channel_id + 1);
	enable_dma(ch->channel_id + 1);

#ifdef DEBUG
		print_vcap0_reg(ch->channel_id);
#endif
	return 0;
}
/* ----------------------------------------------------------------------- */
...
static void vcap_stop_streaming(struct vb2_queue *vq)
{
    ...
    stop_capture(ch->channel_id + 1);
	disable_dma(ch->channel_id + 1);
...
static void vcap_schedule_next_buffer(struct common_obj *common,int dev_id)
{
	...
	common->set_addr(addr+1920*2,0x230000000,dev_id+1);
}
...
static irqreturn_t vcap_channel_isr(int irq, void *dev_id)
{
		...
		gpt_intr(channel_id);
#endif
        if (vcap_intr_status(channel_id)&vcap_intr_status(channel_id + 1)){
...
void init_vcap(int dev_id,int index){
    ...
    set_rgb_type(vcap_ch_params[dev_id][index].rgb_type,dev_id);
    set_hdat_high_stride_for_2channel(vcap_ch_params[dev_id][index],dev_id);
...
static __init int vcap_probe(struct platform_device *pdev)
{
...
#ifdef VIDEO_DEBUG
	printk("baseo 0x%x, base1 0x%x,base2 0x%x====================\n", (u32)res->start,(u32)res1->start,(u32)res2->start);
#endif
	vcap_dev = &pdev->dev;
	...
	init_vcap(VCAP_CHANNEL_0,VIDEO_2K30H);
	init_vcap(VCAP_CHANNEL_1,VIDEO_2K30H);

#endif
#endif
	    res = platform_get_resource(pdev, IORESOURCE_IRQ, res_idx); 
       if(res){
