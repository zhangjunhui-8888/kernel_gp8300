/*
 * pci-gpt-msix.c: a pcie host controller for msi-x interrupt handle
 *
 * Copyright (C) 2018
 *
...
static int gpt_msi_alloc(struct gpt_msi *chip)
{
        ...
        msi = find_first_zero_bit(chip->used, INT_PCI_MSIX_NR);
        if (msi < INT_PCI_MSIX_NR)
...
static irqreturn_t gpt_pcie_msix_irq(int irq, void *data)
{
	unsigned long reg;
	unsigned long flags;	
	unsigned int offset;
	...
	local_irq_save(flags);
        ...
        while ((reg = pciec_reg_read(pcie, GPT_PCIE_MSIX_STA))) {
		for_each_set_bit(offset, &reg, INT_PCI_MSIX_NR) {
			pciec_reg_write(pcie, GPT_PCIE_MSIX_STA, BIT(offset));
                               ...
                               struct msi_desc *desc)
{
	...
	phys_addr_t msg_addr;
        ...
        msg.address_lo = (msg_addr & 0xffffffff) + hwirq*8;
        msg.data = hwirq;
...
static int gpt_pcie_enable_msix(struct gpt_pciec *pcie)
{
        ...
        msi->domain = irq_domain_add_linear(NULL, INT_PCI_MSIX_NR,
        ...
        err = request_irq(msi->irq, gpt_pcie_msix_irq, IRQF_SHARED | IRQF_NO_THREAD,
        ...
        /* setup MSI-X/ data target */
        msi->pages = __get_free_pages(GFP_KERNEL, 0);
        base = virt_to_phys((void *)msi->pages);

        /* enable MSI  32 vector */
        pciec_reg_write(pcie, GPT_PCIE_MSIX_IRQ_EN, 0xff);
	
	for (i = 0; i< INT_PCI_MSIX_NR; i++) {
       		pciec_reg_write(pcie, GPT_PCIE_MSIX_EHADDR(i),  base >> 32);
        	pciec_reg_write(pcie, GPT_PCIE_MSIX_ELADDR(i), (base & 0xffffffff) + i*8);
        	pciec_reg_write(pcie, GPT_PCIE_MSIX_EDATA(i), i);
	}
        reg = pciec_rcw_read(pcie, GPT_PCIE_MSIX_CTRL);
	/*message capable/enable : 8*/
        pciec_rcw_write(pcie, GPT_PCIE_MSIX_CTRL, reg | (0x7 << 16) | 0x1 << 31);
	/*table offset: BAR=0, base lower address*/
        pciec_rcw_write(pcie, GPT_PCIE_MSIX_TABLE, base & 0xfffffff8);
